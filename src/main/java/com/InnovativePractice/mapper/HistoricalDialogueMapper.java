...
public interface HistoricalDialogueMapper {
    ...
    @Select("select sequence_id from historical_dialogue_info where user_id=#{userId} order by date DESC limit 1")
...
""")
    ...
    @Select("select historical_dialogue_info.question,historical_dialogue_info.responds,historical_dialogue_info.date from historical_dialogue_info where sequence_id=#{sequenceId} and user_id=#{userId} and question!= \"\" and responds!=\"\"")
